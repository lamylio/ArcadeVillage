...
public class UnitPlayer : MonoBehaviour
{
    ...
    void FixedUpdate(){       
        ...
        if (_isWalking) _controller.SimpleMove(transform.TransformDirection(Vector3.forward) *   _currentSpeed * _currentMovement.z);
        transform.Rotate(0, _currentMovement.x * _turnSpeed, 0);
    ...
    void onWalkInputs(InputAction.CallbackContext context){
        ...
        _currentMovement = new Vector3(walkMovementInput.x, 0, walkMovementInput.y);
    }

    void onGameStateChanged(State newState){
        if (newState.Is("FreeWalkPlayer")) {
            _inputs.Enable();
            transform.GetChild(0).gameObject.SetActive(true); // Enable the camera (TODO: change to the reference)
